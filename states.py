...
for state in states:
	state_abbreviation.append(state[0].replace("'",""))
	state_name.append(state [1].replace("'",""))
...
print "<select>"
for abbrev, name in zip(state_abbreviation,state_name):
    print """<option value={0}>{1}</option>""".format(abbrev,name)
print "</select>"
# Step 4: Test it!
